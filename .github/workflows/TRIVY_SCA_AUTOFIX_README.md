...
- **Trivy cache** — Scans disable Trivy action caching and use `/tmp/trivy-cache` so cache files are not mistaken for extra repo changes. Both scans skip `node_modules` and `**/node_modules` (merged with `trivy_skip_dirs`).
